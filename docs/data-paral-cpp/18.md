...
如果 C++ std 库提供了一个函数，如图 [18-8](#Fig8) 所列，以及一个 SYCL 内置函数，那么 DPC++ 程序员可以使用其中任何一个。图 [18-1](#Fig1) 展示了主机和设备的 C++ `std::log`函数和 SYCL 内置`sycl::log`函数，两个函数产生相同的数值结果。在示例中，内置的关系函数`sycl::isequal`用于比较`std:log`和`sycl:`的结果。
