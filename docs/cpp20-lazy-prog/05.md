...
如果你想知道`int`、`long int`等等到底有多大，你可以使用`#include <`、`climits`、`>`找到答案，它定义了各种类型的最大值和最小值的常量。你可以用`sizeof` : `sizeof (int)`或者`sizeof (myInt)`得到其中一个的大小，以字节 <sup>[3](#Fn3)</sup> 表示，其中`myInt`是一个`int`。
