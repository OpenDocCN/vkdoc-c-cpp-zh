...
| `unsigned int`<sup>[1](#Fn1)</sup>`strlen (const char myArray[]);` | 返回`myArray`中字符串的长度(到空字符为止有多少个字符)。 |
...
`strcpy_s`和`strcat_s`是`strcpy`和`strcat`的版本，它们试图阻止你写超出数组的界限。听起来很明智，但这通常不会流行起来。我不使用它们，因为我希望代码可以在编译器之间移植。或者我只是喜欢生活在边缘。<sup>2[2](#Fn2)</sup>
