...
在 SSDL，例 [29-1](#PC1) 中的初始化代码是由`SSDL_Display`和`SSDL_SoundSystem`的构造器完成的。示例 [29-1](#PC1) 有所简化。一个大问题是，我们不能在没有 SSDL 的情况下发射`SSDL_Exception`(咄)，所以我们用`return -1;`来处理发射失败。
