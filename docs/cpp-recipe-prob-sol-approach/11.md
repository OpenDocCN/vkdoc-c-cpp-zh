...
![Image](img/sq.jpg) **注意**[清单 11-5](#list5) 中的代码实际上并不强制限制；当达到限制时，它简单地退回到动态分配。
...
这个程序不能阻止`ThreadTask`中的代码和`main`函数访问`MyManagedObject class` 中的`s_ManagedObjects`和`s_FreeList`池。对这些对象的访问可以由一个**互斥体**来保护，正如你在[清单 11-9](#list9) 中看到的。
...
![Image](img/sq.jpg) **注意**[清单 11-11](#list11) 中`unique_lock`的使用在技术上是低效的。持有锁的时间长于从**队列**中检索要创建的对象数量的时间，这实质上是通过在一个**线程**创建对象时使所有**线程**同步来串行化对象的创建。这个例子设计得不好，是为了展示这些对象在实践中是如何使用的。
...
尽管打包的任务消除了对`thread`函数的需求，但您仍然必须手动创建自己的**线程**。C++ 提供了第四层抽象，让你不必担心 ?? 线程。[清单 11-15](#list15) 使用`async`函数异步调用一个函数。
