...
在`main`函数的第一行创建了一个`SharedMyClass`的实例。这个实例用一个动态分配的`MyClass`对象初始化。`MyClass`对象本身用值 10 初始化。在`main`的主体中唯一的其他代码是`return`语句。尽管如此，[图 10-7](#Fig7) 显示`MyClass`的析构函数已经在`sharedMyClass`中存储的对象上被调用。
...
![Image](img/sq.jpg) **注意** [清单 10-14](#list14) 包含调试代码，允许打印对象状态，以便于说明。该代码可以从工作解决方案中删除。
