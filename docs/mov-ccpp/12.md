...
在`main( )`中，使用了一种新型的`iostream`(在`<sstream>`中声明)。这是一个接受一个`string`(它可以从一个`char`数组中创建，如此处所示)并将其转换成一个`iostream`的类。在[清单 12-9](#list9) 的例子中，这意味着不用打开文件或在命令行上输入数据就可以测试移位操作符。
