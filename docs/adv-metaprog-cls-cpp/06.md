...
第一个“循环”不太通用，也更详细。它与您正在使用的容器(即 vector <string>)紧密相关。如果你替换了数据结构，这段代码就不能再编译了。此外，它会在每次迭代中重新计算一次 v.end()。[<sup class="calibre7">1</sup>](#Fn1)</string>
...
*   无论*i 返回什么，指向值的副本都可以存储为 STD::iterator _ traits<T>:::value _ type；通常，您会进一步强调这种类型是可分配的或可交换的。[<sup class="calibre7">3</sup>](#Fn3)
...
![9781484210116_Fig06-01.jpg](img/image00577.jpeg)
...
这个处方听起来像是重新排序，所以你可以考虑一个变异算法。您对输入范围重新排序并返回一个迭代器 h，这样[begin，h]和[h，end 就是所需的分区。另外，作为一个额外的奖励，您可以计算两个分区之和的差![image](img/image00578.jpeg)，这是要最小化的目标。因此，结果将是 std::pair <迭代器，value_type >。
...
![Image](img/image00563.jpeg) **注意**测试一个范围是否已经排序需要线性时间。
